...
def master_schedule(request: HttpRequest) -> HttpResponse:
    student_names_and_unit_ids = get_current_students().filter(legit=True).values(
        'pk', 'user__first_name', 'user__last_name', 'curriculum')
    ...
    unit_dicts = Unit.objects.order_by('position').values('position', 'pk', 'group__subject',
                                                            'group__name', 'code')
...
class UpdateInvoice(LoginRequiredMixin, StaffuserRequiredMixin,
    ...
    if StudentRegistration.objects.filter(user=request.user, container=container).exists():
        ...
        if form.is_valid():
            ...
            if passcode.lower() != container.passcode.lower():
                ...
                messages.error(
                    request, message="That track is not currently accepting registrations.")
            else:
        ...
        if container.allowed_tracks:
            ...
            if most_recent_reg is not None:
                for k in ('parent_email', 'graduation_year', 'school_name', 'aops_username',
                            'gender'):
...
def giga_chart(request: HttpRequest, format_as: str) -> HttpResponse:
    ...
    for invoice in queryset:
        ...
        rows.append([
            student.pk,
            user.username,
            '; '.join(f"{d.extra_data['username']}#{d.extra_data['discriminator']}"
