...
def portal(request: AuthHttpRequest, student_id: int) -> HttpResponse:
    ...
    context['history'] = Student.objects.filter(
        user=student.user).order_by('semester__end_year').values('pk', 'semester__end_year')
...
def resubmit_pset(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if pset.status in ('A', 'PA'):
        ...
        form.fields['next_unit_to_unlock'].queryset = get_units_to_unlock(
            student)  # type: ignore
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if pset.rejected:
            ...
            pset.status = 'PA'
        pset.save()
        messages.success(request, "The problem set is submitted successfully "
                            "and is pending review!")
        logger.log(
            VERBOSE_LOG_LEVEL, f"{student} re-submitted {pset.unit}",
            extra={'request': request})
        return HttpResponseRedirect(pset.get_absolute_url())
...
def index(request: AuthHttpRequest) -> HttpResponse:
    ...
    context['submitted_registration'] = StudentRegistration.objects.filter(
        user=request.user, container__semester__active=True).exists()
    context['exists_registration'] = RegistrationContainer.objects.filter(
        semester__active=True,).exists()
