...
from .models import OIMEAttempt, OIMEProposal, OIMESolverRole


class ProposalListView(VerifiedRequiredMixin, ListView[OIMEProposal]):
    model = OIMEProposal
    template_name = "oime/proposal_list.html"
...
def role_select(request: HttpRequest) -> HttpResponse:
    ...
    return render(request, "oime/role_select.html", {"role": role})


def _get_solver_context(user: Any, proposal: OIMEProposal) -> dict[str, Any]:
    """Return role/attempt info and derived permission flags for a solver."""
    try:
    ...
    can_see_solution = not is_serious or (
        attempt is not None and attempt.is_complete
    )
...
def proposal_detail(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    from .models import OIMEComment
