            ...
            if m_start is not None:
                ...
                if tag_name == 'diamond':
                    try:
                        ...
                        num_found = AchievementUnlock.objects.filter(
                            achievement=diamond).count() or 0
                    ...
                    try:
                        ...
                        num_psets = PSet.objects.filter(
                            unit__group=unitgroup, status__in=('A', 'PA')).count()
                        clubs_given = PSet.objects.filter(
                            unit__group=unitgroup,).aggregate(Sum('clubs'))['clubs__sum'] or 0
