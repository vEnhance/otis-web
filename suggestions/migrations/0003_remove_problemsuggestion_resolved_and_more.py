...
def unset_status(apps: Any, scheme_editor: Any):
    ProblemSuggestion = apps.get_model('suggestions', 'ProblemSuggestion')
    ProblemSuggestion.objects.filter(status__in=('SUGG_NEW',
                                                    'SUGG_EDIT')).update(resolved=False)
    ProblemSuggestion.objects.filter(status__in=('SUGG_NOK', 'SUGG_OK',
                                                    'SUGG_REJ')).update(resolved=False)
    ...
    operations = [
        migrations.AddField(
            ...
            field=models.CharField(
                choices=[('SUGG_EDIT', 'Edits requested'), ('SUGG_NEW', 'Pending'),
                            ('SUGG_NOK', 'Nice rejection'), ('SUGG_OK', 'Accepted'),
                            ('SUGG_REJ', 'Rejected')],
                default='SUGG_NEW',
